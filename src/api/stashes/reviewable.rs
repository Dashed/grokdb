...
pub struct ReviewableStash {
    pub stash_id: i64,
    pub grokdb: Arc<GrokDB>
}

impl ReviewableStash {

    // check if the card is within the stash
    pub fn has_card(&self, card_id: i64) -> Result<bool, QueryError> {

        let ref grokdb = self.grokdb.deref();

        let db_conn_guard = grokdb.stashes.db.lock().unwrap();
    ...
    fn __get_cached_card(&self) -> Result<Option<i64>, QueryError> {
        ...
        // ensure a cache entry exists for this stash
        let ref query_count = format!("
            SELECT
                COUNT(1)
            FROM CachedStashReview
            WHERE
                stash = :stash_id
            LIMIT 1;
        ");
...
impl ReviewableSelection for ReviewableStash {
    ...
    fn number_of_cards(&self) -> Result<i64, QueryError> {
        ...
        match self.grokdb.deref().cards.count_by_stash(self.stash_id) {
            Err(err) => {
                return Err(err);
            },
            Ok(num_cards) => {
    ...
    fn cache_card(&self, card_id: i64) -> Result<(), QueryError> {
        ...
        let ref query = format!("
            INSERT OR REPLACE INTO CachedStashReview(stash, card)
            VALUES (:stash_id, :card_id);
        ");
    ...
    // remove stash/card review entry by stash
    fn remove_cache(&self) -> Result<(), QueryError> {
        ...
        let ref query_delete = format!("
            DELETE FROM CachedStashReview WHERE stash = :stash_id;
        ");
    ...
    fn remove_cached_card(&self, card_id: i64) -> Result<(), QueryError> {
        ...
        let ref query_delete = format!("
            DELETE FROM CachedStashReview WHERE card = :card_id;
        ");
